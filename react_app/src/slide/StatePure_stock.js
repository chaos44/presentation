...
function StatePure (props) {
  ...
  const createState = (state) => {
      return new Proxy(state, {
        ...
        set(target, property, value) {
          target[property] = value; // 変更した後の値を、元のオブジェクトに代入（これで単方向バインディングを実現、素晴らしい）
  ...
  const render = () => {
    const bindings = Array.from(document.querySelectorAll('[data-binding]')).map(
      e => e.dataset.binding // data-xxxで指定した要素はdatasetで取得できる(便利！getAttritube()は不要！一気に取得もできるし)
    );
    // console.log(bindings); // この例では Array [ "name", "title" ]
    ...
    // jsでテーブルを作成する。テーブルに挿入する要素(オブジェクト)と表示の変化を追加する
  };
  
  render();
  // 疑問：普通のclassでも効く？
  // 解決、classNameを使う必要がある。さもないとエラー出る。再起動すると表示おかしくなることがある
